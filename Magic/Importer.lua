-- By Amuzet
mod_name,version='Card Importer',2.001
...
local Deck, Tick, Test, Quality, Back = 1, 0.2, false, TBL.new('normal', {}),
    TBL.new('https://i.stack.imgur.com/787gj.png', {})
        ...
        success, errorMSG = pcall(function()
            ...
            if c.card_faces and c.image_uris then -- Adventure/Split
                ...
                for i, f in ipairs(c.card_faces) do
                    ...
                    if not c.oracle:find('Aftermath') and ('InstantSorcery'):find(f.type_line) then
            ...
            elseif c.card_faces and not c.image_uris then -- DFC REWORKED for STATES!
                ...
                if t.image then
                    faceAddress, backAddress = t.image, t.image
                end
                c.face = faceAddress
...
function onChat(msg, p)
    if msg:find('!?[Ss]cryfall ') then
        ...
        if a == 'hide' and p.admin then
            ...
            broadcastToAll('Importer now ' .. msg ..
                               ' Chat messages with Importer in them.\nToggle this with "Scryfall Hide"', SMC)
        elseif a == 'help' then
            p.print(Usage, {0.9, 0.9, 0.9})
            return false
...
"76561198317076000":"https://i.imgur.com/vh8IeEn.jpeg"}]], '\n', '')
            ...
            Back = TBL.new('https://gamepedia.cursecdn.com/mtgsalvation_gamepedia/f/f8/Magic_card_back.jpg',
                JSON.decode(self.script_state))
...
function registerModule()
    ...
    if enc then
        ...
        function eEmblemAndTokens(o, p)
            ENC(o, p, 'Token')
        end
        function eOracle(o, p)
            ENC(o, p, 'Text')
        end
        function eRulings(o, p)
            ENC(o, p, 'Rules')
        end
        function ePrintings(o, p)
            ENC(o, p, 'Print')
        end
        function eRespawn(o, p)
            ENC(o, p, 'Spawn')
        end
        function eSetSleeve(o, p)
            ENC(o, p, 'Back')
        end
